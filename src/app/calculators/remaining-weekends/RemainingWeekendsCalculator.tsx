...
function Calculator() {
  ...
  const result = useMemo(() => {
    ...
    const remainingYears = r - a;
    const weekends = remainingYears * 52;
    const alreadyUsed = a * 52;
    const percentUsed = (alreadyUsed / (r * 52)) * 100;
    return { remainingYears, weekends, alreadyUsed, percentUsed };
  }, [age, retireAge]);
    ...
    <div className="space-y-6">
      ...
      {result && (
        <div className="border-t pt-6 space-y-4">
          ...
          <ShareButton text={`은퇴까지 남은 주말 ${formatNumber(result.weekends)}번. 이미 ${formatNumber(result.alreadyUsed)}번 씀.`} />
        </div>
      )}
    </div>
  );
