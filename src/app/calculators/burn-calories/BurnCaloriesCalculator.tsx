...
const FOODS = [
  ...
  { name: "짜장면 1그릇", cal: 660 },
  { name: "삼겹살 1인분 (200g)", cal: 580 },
  { name: "떡볶이 1인분", cal: 480 },
  { name: "아메리카노 (라지)", cal: 15 },
  { name: "카페라떼 (라지)", cal: 220 },
  { name: "소주 1병", cal: 540 },
  { name: "맥주 500ml", cal: 215 },
...
export function BurnCaloriesCalculator() {
  ...
  const result = useMemo(() => {
    return EXERCISES.map((ex) => ({
      ...ex,
      minutes: Math.round(food.cal / ex.calPerMin),
    }));
  }, [food]);
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-px rounded-lg border overflow-hidden">
          {FOODS.map((f, i) => (
            <button
              ...
              className={`w-full flex justify-between px-3 py-2.5 text-sm transition-colors ${
                selected === i
      ...
      <div className="border-t pt-6 space-y-4">
        <p className="text-lg font-semibold">
          {food.name} ({formatNumber(food.cal)}kcal)을 태우려면
        </p>
        ...
        <ShareButton text={`${food.name} (${formatNumber(food.cal)}kcal) 태우려면 달리기 ${result[0].minutes}분.`} />
      </div>
    </div>
  );
}
